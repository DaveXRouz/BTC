...
def scalar_multiply(k, point=None):
    """Compute k * point using double-and-add. Default point is G."""
    if point is None:
        point = G
    if k == 0:
        return ECPoint(0, 0)
...
PUZZLES = {
    ...
    20: {
        'address':     '1HsMJxNiV7TLxmoF6uJNkydxPFDog4NQL3',
        'range_start': 2**19,
        'range_end':   2**20 - 1,
        'public_key':  None,
        'type':        'A',
    ...
    25: {
        'address':     '1GXFC2ZbBMkMPsGBHawwo7cNpecVWo47Aj',
        'range_start': 2**24,
        'range_end':   2**25 - 1,
        'public_key':  None,
        'type':        'A',
    ...
    35: {
        'address':     '1C5bSj1iEGUgSTbziymG7Cn18ENQuT36vv',
        'range_start': 2**34,
        'range_end':   2**35 - 1,
        'public_key':  None,
        'type':        'A',
    ...
    40: {
        'address':     '1EeAxcprB2PpCnr5iCQP7EwSejz9pcnHo6',
        'range_start': 2**39,
        'range_end':   2**40 - 1,
        'public_key':  None,
        'type':        'A',
    ...
    75: {
        'address':     '1KWwP9EBpBg49bXkQcHeJY3JbVHEMooJgs',
        'range_start': 2**74,
        'range_end':   2**75 - 1,
    ...
    80: {
        'address':     '1EJFhahqdSiDm1EP3VZnXvM3AaZ1kiBvtR',
        'range_start': 2**79,
        'range_end':   2**80 - 1,
        'public_key':  None,
        'type':        'B',
    ...
    85: {
        'address':     '1LVKXE9FRjm2hpXczYksTZ8pDETVJHwq1V',
        'range_start': 2**84,
        'range_end':   2**85 - 1,
        'public_key':  None,
        'type':        'B',
    ...
    90: {
        'address':     '12PEd8DTfMUnGZBnjn9sRXMiPCYGQhzABM',
        'range_start': 2**89,
        'range_end':   2**90 - 1,
        'public_key':  None,
        'type':        'B',
...
def get_performance_estimate(puzzle_num, algo='auto'):
    ...
    if algo == 'auto':
        algo = 'kangaroo' if puz['type'] == 'B' and puz.get('public_key') else 'bruteforce'
