...
Backups are stored in `backups/oracle/` as gzipped SQL files. Retention: 30 backups per type (full/data-only).
...
Variables are loaded from `.env` if present, or can be set in the shell environment.

## Full Database Backup

For full database backup/restore (all tables), use the scripts in `scripts/`:

```bash
scripts/backup.sh      # Full database backup
scripts/restore.sh      # Full database restore (drops and recreates DB)
```
